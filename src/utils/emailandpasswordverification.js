...
const emailandpassverification = async (email, password) => {
  try {
    ...
    if (!document) {
      return res.status(401).send("invalid credentials"); // instead of if else if use return to stop the execution
    }

    const isverifedUser = await bcrypt.compare(password, document.password);
    if (!isverifedUser) {
      return res.status(401).send("invalid credentials"); // im not exposing which one is wrong to prevent data breach /leak
    }
